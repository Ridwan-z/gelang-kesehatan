...
import 'package:gelang_kesehatan/features/profile/screens/alert_threshold_screen.dart';
...
// ── Provider: ambil profil dari Supabase ──────────────────────
...
class ProfileScreen extends ConsumerWidget {
  ...
  void _showEditSheet(
    ...
    showModalBottomSheet(
      ...
      builder: (_) => _EditProfileSheet(current: current, ref: ref),
    );
  }
}
    ...
    return Container(
      ...
      child: ListTile(
        leading: Container(
          ...
          decoration: BoxDecoration(
            color:
                (isDark ? Colors.indigo : Colors.orange).withOpacity(0.12),
            borderRadius: BorderRadius.circular(10),
          ),
          child: Icon(
...
class _EditProfileSheetState extends ConsumerState<_EditProfileSheet> {
  ...
  Future<void> _save() async {
    ...
    if (_nameCtrl.text.trim().isEmpty) {
      setState(() => _errorMsg = 'Nama lengkap tidak boleh kosong');
      return;
    }
    ...
    try {
      ...
      // Parse nilai numerik dengan aman
      ...
      // 1. Update tabel profiles di Supabase
      ...
      // 2. Update display name di auth metadata
      await supabase.auth.updateUser(
        UserAttributes(data: {'full_name': name}),
      );

      // 3. Invalidate provider supaya UI refresh otomatis
      ...
      setState(() {
        _isSaving = false;
        _errorMsg = 'Gagal menyimpan: ${e.toString().replaceAll('Exception: ', '')}';
      });
    }
  }
}